#!/usr/bin/env python3
"""
generate_cards.py — KUD: De Waardeloze Therapeut
...
- Ontbreekt de afbeelding? Dan komt er automatisch een PLACEHOLDER met de gewenste naam,
  zodat het deck nu al speelbaar/testbaar is en je weet welke art je nog nodig hebt.

Gebruik:
    python3 generate_cards.py            # bouwt fronts + backs voor alle types
    python3 generate_cards.py --no-backs # alleen de voorkanten
...
# Type -> (Nederlandse naam, kleur, of het een beeldkaart is)
TYPES = {
    "patient":     {"label": "PATIËNT",     "color": "#2e7d32", "image": True},
    "klacht":      {"label": "KLACHT",      "color": "#1565c0", "image": False},
    "behandeling": {"label": "BEHANDELING", "color": "#c62828", "image": True},
    "speciaal":    {"label": "SPECIAAL",    "color": "#6a1b9a", "image": True},
}

CSV_FILES = {
    "patient":     "patienten.csv",
    "klacht":      "klachten.csv",
    "behandeling": "behandelingen.csv",
...
def render_card(card: dict) -> str:
    ...
    # "Gouden Diploma"-rand voor topkaarten (rubricscore >= 18)
    golden = ""
    try:
    ...
    # Beeldgebied: echte afbeelding of placeholder
    if meta["image"]:
        if asset_exists(fragment):
            art = f'<div class="art"><img src="assets/{esc(fragment)}" alt="{esc(fragment)}"></div>'
        else:
            want = esc(fragment) if fragment else "(geen fragment opgegeven)"
            art = (
                '<div class="art placeholder">'
                '<div class="ph-icon">🖼️</div>'
                '<div class="ph-text">FRAGMENT NODIG</div>'
                f'<div class="ph-name">{want}</div>'
                "</div>"
            )
    else:
        # Tekstkaart (klacht): grote tekst, geen beeld
        art = f'<div class="art textcard"><div class="bigtext">{bijschrift}</div></div>'

    # Onderste tekstbalk
    if ctype == "klacht":
        caption = ""  # tekst staat al groot in het beeldgebied
    elif ctype == "speciaal":
        caption = (
        ...
        caption = f'<div class="caption">{bijschrift}</div>'
    ...
    return (
        f'<div class="card{golden}" style="--c:{color}">'
        f'<div class="band">{meta["label"]}</div>'
        f"{art}{caption}{footer}"
        "</div>"
    )
...
def render_back(ctype: str) -> str:
    meta = TYPES[ctype]
    return (
        f'<div class="card back" style="--c:{meta["color"]}">'
        f'<div class="back-inner"><div class="back-logo">KUD</div>'
        f'<div class="back-sub">De Waardeloze Therapeut</div>'
        f'<div class="back-type">{meta["label"]}</div></div>'
        "</div>"
    )
...
.sheet {
  width: 210mm; min-height: 297mm; padding: 8mm; margin: 0 auto 6mm;
  background: #fff; display: grid; grid-template-columns: repeat(3, 63mm);
  grid-auto-rows: 88mm; gap: 4mm; justify-content: center; align-content: start;
}
.section-title { width: 210mm; margin: 10mm auto 2mm; font-weight: 800; color: #333; }
.card {
  width: 63mm; height: 88mm; border: 0.4mm solid var(--c); border-radius: 3mm;
...
.band {
  background: var(--c); color: #fff; font-weight: 800; font-size: 3mm;
  letter-spacing: 0.6mm; padding: 1.4mm 2.5mm; text-align: center;
}
.art { flex: 1; display: flex; align-items: center; justify-content: center; overflow: hidden; background:#fafafa; }
.art img { width: 100%; height: 100%; object-fit: cover; }
.art.placeholder {
  flex-direction: column; gap: 1.5mm; color: var(--c);
  background: repeating-linear-gradient(45deg, #f3f3f3, #f3f3f3 3mm, #ececec 3mm, #ececec 6mm);
  text-align: center; padding: 3mm;
}
.ph-icon { font-size: 9mm; }
.ph-text { font-weight: 800; font-size: 3mm; letter-spacing: 0.4mm; }
...
.footer {
  ...
  padding: 1.5mm 2.5mm; font-size: 2.4mm; color: #888; border-top: 0.2mm solid #eee;
}
...
def build_html(cards: list[dict], with_backs: bool) -> str:
    parts = [
        "<!DOCTYPE html><html lang='nl'><head><meta charset='utf-8'>",
        "<title>KUD: De Waardeloze Therapeut — printvel</title>",
        f"<style>{CSS}</style></head><body>",
    ]

    order = ["patient", "klacht", "behandeling", "speciaal"]
    by_type: dict[str, list[dict]] = {t: [] for t in order}
    for c in cards:
        by_type.setdefault(c["type"], []).append(c)

    # Voorkanten, per type gegroepeerd
    for t in order:
        group = by_type.get(t, [])
        if not group:
            continue
...
def main() -> int:
    ap = argparse.ArgumentParser(description="Genereer print-klaar HTML-vel voor KUD: De Waardeloze Therapeut.")
    ap.add_argument("--no-backs", action="store_true", help="Sla achterkanten over.")
    args = ap.parse_args()

    print("KUD: De Waardeloze Therapeut — kaartgenerator")
    ...
    missing = sum(
        1 for c in cards
        if TYPES.get(c["type"], {}).get("image") and not asset_exists((c.get("fragment") or "").strip())
    )
